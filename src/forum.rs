            ...
            if let Some(course_id) = course_id {
                sqlx::query_as::<_, ThreadRow>(
                    "SELECT ft.id,
                            ...
                            TO_CHAR(ft.created_at, 'DD Mon YYYY') AS created_at,
                            TO_CHAR(COALESCE((SELECT MAX(fp.created_at) FROM forum_posts fp WHERE fp.thread_id = ft.id), ft.updated_at), 'DD Mon YYYY') AS last_reply_at,
...
async fn load_thread(db: &PgPool, thread_id: i32, user_id: i32) -> Option<ForumThreadDetail> {
    let row = sqlx::query_as::<_, ThreadDetailRow>(
        "SELECT ft.id,
                ...
                u.display_name AS author,
                UPPER(SUBSTRING(u.display_name, 1, 1)) AS author_initials,
                TO_CHAR(ft.created_at, 'DD Mon YYYY HH24:MI') AS created_at,
                TO_CHAR(ft.updated_at, 'DD Mon YYYY HH24:MI') AS updated_at,
                CASE WHEN ft.edited_at IS NULL THEN NULL ELSE TO_CHAR(ft.edited_at, 'DD Mon YYYY HH24:MI') END AS edited_at,
                CASE WHEN ft.deleted_at IS NULL THEN NULL ELSE TO_CHAR(ft.deleted_at, 'DD Mon YYYY HH24:MI') END AS deleted_at,
                du.display_name AS deleted_by_name,
                ft.delete_reason,
                CASE WHEN ft.locked_at IS NULL THEN NULL ELSE TO_CHAR(ft.locked_at, 'DD Mon YYYY HH24:MI') END AS locked_at,
...
async fn load_posts(
    ...
    let rows = sqlx::query_as::<_, ForumPostFlat>(
        "SELECT fp.id,
                ...
                TO_CHAR(fp.updated_at, 'DD Mon YYYY HH24:MI') AS updated_at,
                CASE WHEN fp.edited_at IS NULL THEN NULL ELSE TO_CHAR(fp.edited_at, 'DD Mon YYYY HH24:MI') END AS edited_at,
                CASE WHEN fp.deleted_at IS NULL THEN NULL ELSE TO_CHAR(fp.deleted_at, 'DD Mon YYYY HH24:MI') END AS deleted_at,
                du.display_name AS deleted_by_name,
                fp.delete_reason,
...
async fn load_moderation_actions(
    ...
    sqlx::query_as::<_, ModerationActionView>(
        "SELECT fma.action,
                ...
                TO_CHAR(fma.created_at, 'DD Mon YYYY HH24:MI') AS created_at
